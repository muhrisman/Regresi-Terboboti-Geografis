---
title: "Regresi Terboboti Geografis Dengan Fungsi Pembobot Kernel Gaussian"
...
Salah satu kajian dalam analisis regresi adalah dengan mempertimbangkan data spasial. Berbeda dengan pemodelan analisis regresi pada umumnya, pemodelan dengan menggunakan data spasial mempertimbangkan aspek kewilayahan yang memberikan pengaruh lingkungan dan karakteristik yang muncul dari suatu wilayah.
...
Regresi terboboti geografis dapat menangani permasalahan yang muncul akibat misinterpretasi saat melakukan pemodelan regresi linear klasik karena adanya keragaman spasial dalam data. Output yang muncul pada pemodelan regresi terboboti spasial adalah setiap observasi yang berbasis ruang memiliki model regresi masing-masing.
...
Vektor observasi variabel target dinyatakan sebagai $y$ sedangkan $\epsilon_{i}^{2}$ adalah kuadrat galat pengamatan ke-$i$ dan $\sigma^{2}$ adalah ragam dari $\epsilon_{i}$
...
1. Critical cut of Neighbourhood
   ...
   Dua lokasi yang memiliki koordinat _easting-northing_ $s_{i}$ dan $s_{j}$ dianggap      bertetangga jika $0\le d_{ij} \le d^{*}$ dengan $d_{ij}$ adalah jarak antarlokasi dan $d^{*}$ mewakili jarak _critical cut off_.
...
saveRDS(peta_kriminalitas, file = "peta_k.rds")
# Restore the object
readRDS(file = "peta_k.rds")
...
Sebelum dilakukan pemodelan dengan regresi terboboti geografis, akan dilakukan pemodelan regresi linear berganda dengan Ordinary Least Square (OLS) untuk mendapatkan model global dari tingkat kriminalitas di pulau Jawa.
...
Adjusted R-Squared yang dihasilkan adalah 58% yang berarti variabel-variabel yang dilibatkan mampu menjelaskan 58% keragaman tingkat kriminalitas di Pulau Jawa sedangkan 42% sisanya berasal dari faktor lain.
...
Untuk mengetahui signifikansi keragaman spasial dalam observasi akan dilakukan uji keragaman spasial menggunakan uji Breusch-Pagan error model global yang sudah terbentuk. Uji ini sama dengan uji homoskedastisitas dalam pengecekan regresi linier berganda.
...
Hal ini disebabkan karena terdapat efek spasial dalam data. Dengan munculnya faktor keragaman spasial mengindikasikan setiap daerah memiliki karakteristiknya masing-masing. Model yang akan digunakan untuk mengidentifikasi karakteristik masing-masing daerah adalah regresi terboboti geografis.
...
Model yang dihasilkan dengan regresi terboboti geografis (RTG) menghasilkan model yang lebih baik dibanding model regresi dengan Ordinary Least Square (OLS) (MKT). Hal ini dapat terlihat dari nilai Adjusted $R^2$ yang lebih besar dan nilai AIC yang lebih kecil pada model RTG dibandingkan model MKT.
...
```{r}
# Save an object to a file
saveRDS(peta_koef_kp, file = "peta_koef_kp.rds")
# Restore the object
readRDS(file = "peta_koef_kp.rds")
```
...
```{r}
# Save an object to a file
saveRDS(peta_koef_pdrb, file = "peta_koef_pdrb.rds")
# Restore the object
readRDS(file = "peta_koef_pdrb.rds")
```
...
mytext_kp_sig <- paste(
  ...
  gwr_clean$TYPE_2," ", gwr_clean$NAME_2, "<br/>",
  "Provinsi ", gwr_clean$Provinsi, "<br/>",
  "Signifikansi P-Value 5% : "," ", gwr_clean$sig_kp_5, "<br/>",
  sep = "") %>% 
  lapply(htmltools::HTML)
```
...
```{r}
# Save an object to a file
saveRDS(peta_koef_kp_sig, file = "peta_koef_kp_sig.rds")
# Restore the object
readRDS(file = "peta_koef_kp_sig.rds")
```
...
mytext_pdrb_sig <- paste(
  ...
  gwr_clean$TYPE_2," ", gwr_clean$NAME_2, "<br/>",
  "Provinsi ", gwr_clean$Provinsi, "<br/>",
  "Signifikansi P-Value 5% : "," ", gwr_clean$sig_pdrb, "<br/>",
  sep = "") %>% 
  lapply(htmltools::HTML)
...
```{r}
# Save an object to a file
saveRDS(peta_pdrb_sig, file = "peta_pdrb_sig.rds")
# Restore the object
readRDS(file = "peta_pdrb_sig.rds")
```
...
```{r}
# Save an object to a file
saveRDS(peta_r2_lokal, file = "peta_r2_lokal.rds")
# Restore the object
readRDS(file = "peta_r2_lokal.rds")
```
...
Analisis regresi terboboti geografis (RTG) dengan fungsi pembobot kernel 
Gaussian adaptive menunjukkan hasil yang lebih baik dibanding analisis regresi klasik dengan Ordinary Least Square (OLS). Hal ini dapat terlihat dari nilai $R^2$ dan AIC yang lebih baik.

Variabel Kepadatan Penduduk (KP) dan Produk Domestik Regional Bruto (PDRB) memiliki koefisien positif dan memiliki p-value signifikan pada taraf nyata 5%.

Beberapa daerah menunjukan p-value tidak signifikan di variabel-variabel yang signifikan saat pemodelan global.

Hasil RTG dapat didekati dengan analisis klaster untuk merangkum karakteristik daerah yang mirip. Pada saat pengklasteran terdapat perbedaan anatara menggunakan koefisien variabel prediktor dan signifikansi p-value.

# Saran

Terdapat beberapa macam fungsi kernel lain yang bisa digunakan untuk melakukan analisis RTG. Beberapa diantaranya adalah kernel Tricube, kernel kuadrat, dan kernel eksponensial. Penggunaan pembobotan kernel yang berbeda dapat dibandingkan untuk melihat kemampuan pembobot yang menghasilkan pemodelan terbaik. Selain itu terdapat beberapa macam model lain dalam pemodelan RTG yaitu RTG Logistik, RTG Lasso dan RTG Ridge yang dapat dikaji lebih lanjut.
...
[^2] Fotheringham AS, Brunsdon C, Charlton M. 2002. Geographically Weighted 
[^3] Regression : the Analysis of Spatially Varying Relationships. West Sussex 
(UK): John Wiley and Sons LTD.

[^4] https://algotech.netlify.app/blog/creating-choropleth-with-mapshaper-and-r/
...
[^7] Kusuma LP. 2018. Regresi Terboboti Geografis dengan Fungsi 
Pembobot Kernel Gaussian pada Kekuatan Sinyal Seluler [skripsi]. Bogor (ID): Institut Pertanian Bogor.

[^8] Mastuti WC. 2016. Pemodelan Regresi Kekar Terboboti Geografis 
(Studi Kasus : Kriminalitas Jawa Timur Tahun 2016) [skripsi]. Bogor (ID): Institut Pertanian Bogor.

[^9] [BPS]. Badan Pusat Statistik.2020.Provinsi Banten Dalam Angka 2020. Serang(ID) : BPS

[^10] [BPS]. Badan Pusat Statistik.2020.Provinsi DKI Jakarta Dalam Angka 2020. Jakarta(ID) : BPS

[^11] [BPS]. Badan Pusat Statistik.2018.Statistik Kesejahteraan Rakyat Provinsi DKI Jakarta 2018. Jakarta(ID) : BPS

[^12] [BPS]. Badan Pusat Statistik.2021.Provinsi DI Yogyakarta Dalam Angka 2021. Yogyakarta(ID) : BPS

[^13] [BPS]. Badan Pusat Statistik.2021.Provinsi Jawa Barat Dalam Angka 2020. Bandung(ID) : BPS

[^14] [BPS]. Badan Pusat Statistik.2021.Provinsi Jawa Tengah Dalam Angka 2020. Semarang(ID) : BPS

[^15] [BPS]. Badan Pusat Statistik.2021.Provinsi Jawa Timur Dalam Angka 2021. Surabaya(ID) : BPS
